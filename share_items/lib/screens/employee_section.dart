...
import 'package:share_items/screens/supply_details.dart';
...
class _MainSectionState extends State<MainSection> {
  var logger = Logger();
  bool online = true;
  late List<Item> medicalSupplies = [];
  ...
  getBooks() async {
    ...
    if (online) {
      try {
        medicalSupplies = await ApiService.instance.getMedicalSupplies();
        DatabaseHelper.updateBooks(medicalSupplies);
      } catch (e) {
        logger.e(e);
        message(context, "Error connecting to the server", "Error");
      }
    } else {
      medicalSupplies = await DatabaseHelper.getMedicalSupplies();
    }
              ...
              children: [
                ListView.builder(
                  itemBuilder: ((context, index) {
                    return ListTile(
                      title: Text(medicalSupplies[index].name),
                      subtitle: Text(
                          '${medicalSupplies[index].id}, ${medicalSupplies[index].supplier}, ${medicalSupplies[index].type}'),
                       onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => SupplyDetailsScreen(
                                supplyId: medicalSupplies[index].id!,
                              ),
                            ),
                          );
                        },
                  ...
                  itemCount: medicalSupplies.length,
